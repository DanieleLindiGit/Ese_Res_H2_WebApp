...
let getTechAndEconomicalParameters (inp: SystemInputs) (elec: ElectrolyzersOutput) =
...
let getCapexCost (inp: SystemInputs) (elec: ElectrolyzersOutput) (tec: TechAndEconomicalParameters) =
    {
      //F14*1000*(Input!B95+Input!B96+Input!B97)
      CapexCost.ElectrolizerEquipment =
          tec.ElectrolizerNominalStackPower
          * 1000.0
          * (inp.Electrolyzers.PriceElectrolyzer
             + inp.Electrolyzers.PriceBOP
             + inp.Electrolyzers.PriceOther)

      //F15*1000*Input!B46
      Batteries =
          tec.BatteriesEnergyCapacity
          * 1000.0
          * inp.Battery.SpecificPrice
      ...
      // Input!B98*F14
      OverhaulElectrolizer =
          inp.Electrolyzers.OverhaulElectrolyzer
          * tec.ElectrolizerNominalStackPower
      ...
      OxigenCompressor =
          inp.Oxigen.Capex
          * // Capex per produrre ossigeno (compressore)
          elec.OxygenProduction
          * elec.NominalH2ProductionTot }

let getBusinessPlanInput (inp: SystemInputs) (fin: FinancialInputs) =
...
let getHydrogenRevenues hydrogenProduction lcoh h2PriceEscalation (totalYear: int) =
    (hydrogenProduction
     * lcoh
     * ((1.0 + h2PriceEscalation / 100.0)
        ** (float totalYear - 1.0)))
    / ThousandsOrUnits

let resetYearAnalisys (inp: YearAnalysis) (lcoh: float) =
    let HydrogenRevenues =
        getHydrogenRevenues inp.HydrogenProduction lcoh inp.H2PriceEscalation inp.TotalYear

    let TotalRevenues =
        HydrogenRevenues
        + inp.OxigenRevenues
        + inp.ExtraEEtoGridRevenues
...
let getYearAnalysis (sys: SystemInputs) (inp: BusinessPlanInput) (yearOfOp: int) (totalDebt: float) =
    let TotalYear =
        inp.FinancialInputs.InitialInvestmentBreakdown.Length
        + yearOfOp
    ...
    let HydrogenProduction =
        cy.Rows
        |> List.sumBy (fun v -> v.TotalH2Production)

    let OxigenProduction = cy.Rows |> List.sumBy (fun v -> v.O2Production)

    let ExtraEEtoGrid =
        (cy.Rows |> List.sumBy (fun v -> v.EnergyToGrid))
        / 1000.0
    ...
    let PResProgrammable =
        cy.Rows
        |> List.sumBy (fun v -> v.PResProgrammable)

    let BatteryCharging = cy.Rows |> List.sumBy (fun v -> v.Charging)

    let WaterConsumption =
        cy.Rows
        |> List.sumBy (fun v -> v.WaterConsumption)

    let BiomassForEEProduction =
        cy.Rows
        |> List.sumBy (fun v -> v.BiomassForEEProduction)

    let OxigenRevenues =
        OxigenProduction * sys.Oxigen.Price
        / ThousandsOrUnits

    let ExtraEEtoGridRevenues =
        ExtraEEtoGrid
        * inp.FinancialInputs.VariableCosts.ElectricityToGrid
        / ThousandsOrUnits

    let FixedCosts =
        inp.OEM_Costs / ThousandsOrUnits
        * inflationUpToDate

    let ElectricityCost =
        (ToElectrolyzer - PResProgrammable
         + BatteryCharging / (sys.Battery.Efficiency / 100.0))
        * inp.FinancialInputs.VariableCosts.ElectricityFromPvAndWind
        / 1000.0
        / ThousandsOrUnits
        * inflationUpToDate
    ...
    let WaterCost =
        (WaterConsumption
         * inp.FinancialInputs.VariableCosts.Water
         * inflationUpToDate)
        / ThousandsOrUnits

    let BiomassCost =
        (BiomassForEEProduction
         * inp.FinancialInputs.VariableCosts.StrawConsumption
         * inflationUpToDate)
        / 1000.0
        / ThousandsOrUnits
...
let getBusinessPlanOutput (inp: SystemInputs) (fin: FinancialInputs) =
...
let finalBusinessPlan (inp: SystemInputs) (fin: FinancialInputs) =
