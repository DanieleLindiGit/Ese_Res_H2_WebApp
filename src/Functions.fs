...
let CalcElectrolizersTablesOutput (el_input: Electrolyzers) (el_output: ElectrolyzersOutput) (yearOfOp: int) =
    ...
    let productionTemp =
        el_output.EnergyConsumptionOutput
        |> List.map (fun e ->
            el_output.DcConsumptionC32 * e.Load
            / 100.0
            / e.ConsumptionDC)
...
let ElectrolyzersWithDegradationStep1 (electrolyzers: Electrolyzers) =
    ...
    let dc32 = electrolyzers.NominalH2Production * electrolyzers.EnergyConsumption.Head.Consumption * 0.976 * hsvn

    { //First Entry
      ElectrolyzersOutput.NominalH2ProductionTot =
        electrolyzers.NominalH2Production
        * float electrolyzers.Lines
      DcConsumptionC32 = dc32
        ...
        |> List.mapi (fun idx ec ->
            { EnergyConsumptionOutput.Load = ec.Load
              ...
              NominalH2Production =
                ...
                | _ -> dc32 * ec.Load / 100.0 / (hsvn * ec.Consumption * 0.976) })
